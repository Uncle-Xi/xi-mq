...
public class Receive1 {
    ...
    static {
        ...
        properties.setProperty(ClientConfig.XIMQ_GROUP_ID, "group-consumer-Receive0");
    }

    public static void main(String[] args) {
        Consumer consumer = new Consumer(properties);
        consumer.subscribe(Collections.singleton("hello"), new RecordFuture() {
            @Override
            public void callback(Response response) {
                System.out.println("Receive0 Receive Send0[response] -> " + StringUtils.getString(response));
            }
        });
    }
}
