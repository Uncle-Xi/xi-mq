...
public class ReceiveMsg2 {
    ...
    static {
        ...
        properties.setProperty(ClientConfig.XIMQ_GROUP_ID, "group-consumer-1");
    }

    public static void main(String[] args) {
        Consumer consumer = new Consumer(properties);
        consumer.subscribe(Collections.singleton("HelloXiMQ"), new RecordFuture() {
            @Override
            public void callback(Response response) {
                System.out.println("ReceiveMsg2[response] -> " + StringUtils.getString(response));
            }
        });
    }
}
