...
public class SendMeg {
    ...
    static {
        ...
        properties.setProperty(ClientConfig.XIMQ_GROUP_ID, "group-producer-1");
    }
    ...
    public static void main(String[] args) {
        ...
        while (true) {
            try {
                executor.submit(() -> {
                    producer.send("HelloXiMQ", "XiMQ-HelloXiMQ-" + i++, new RecordFuture() {
                        @Override
                        public void callback(Response response) {
                            System.out.println("SendMeg[response] -> " + StringUtils.getString(response));
                        }
                    });
                    if (i % 10 == 0) {
